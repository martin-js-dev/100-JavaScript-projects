...
document.getElementById('resetBooks').addEventListener('click', resetBookCards);
...
function changeBook(e) {
  if (e.target.classList.contains("removeButton")) {
    ...
    let bookCardToRemove = document.querySelector(`[data-index="${index}"]`);
    container.removeChild(bookCardToRemove);
    myLibrary.splice(index, 1);
  }
  else if (e.target.classList.contains("readStatusBtn")) {
    let index = e.target.getAttribute('data-index');
    let bookCardToChange = document.querySelector(`[data-index="${index}"]`);
    let readStatus = bookCardToChange.querySelector('.isReadStatus');
...
function displayBooks() {
  makeBookCard(myLibrary[myLibrary.length - 1]);
}

// DISPLAY CARD ON UI
function makeBookCard(bookToAdd) {
  ...
  bookCard.setAttribute("data-index", myLibrary.indexOf(bookToAdd));
